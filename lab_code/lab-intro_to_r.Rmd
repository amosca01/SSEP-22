...
We will be using R Markdown to create reproducible lab reports, which are a pretty close analog to the analyses and reports you will ultimately produce here at MassMutual. See the
...
> Note that not all data objects listed by `data()` are `data.frame`s! Some of them are `list`s and some have class `matrix`.
...
Using this function, you can read a CSV file from your computer, or as straight from the Internet via a [URL](https://en.wikipedia.org/wiki/URL). CSVs should have the file extension `.csv`. To get you started, run the following command to load the data from a comma-separated value (CSV) file hosted online:
...
specifically, revenue and advertising data from _Advertising Age_'s 14th annual Magazine 300 Report, which was released in September 2003. You should see that the 
workspace area in the upper righthand corner of the RStudio window now lists a 
dataset called `magazines` that has 204 observations on 5 variables. 

As you interact with R, you will create a series of objects. Sometimes you'll load them from a file as we have done here, and sometimes you'll create them yourself as the byproduct of a 
...
dataset in the *Data Viewer* (upper left window), which looks something like this:
...
Noticethat the command above again looks like a function, this time with three arguments
...
`total`. We'll learn a bunch more about piping this afternoon.
</div>
