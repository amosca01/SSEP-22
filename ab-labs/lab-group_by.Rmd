...
The key to solving this problem is to recognize that we need to collapse the **two** rows corresponding to each assigned sex in each year into a single row that contains the information for both sexes (i.e. `summarize()`). Unfortunately, there is no way for R to know what to compute its own---we have to tell it. 
...
Thus, the variables defined by the arguments to `summarize()` usually involve *aggregate* functions like `sum()`, `mean()`, `length()`, `max()`, `n()`, etc. In this case, we can use the conditional `ifelse()` statement around a `sum()` to extract the number of `F` babies born in each year [this will be our numberator], and another one to extract the total number of `M` babies bown in that same year [this will be our denominator]:
...
```
Now all that's left is to look for the year with a `ratio` that's closest to 0.5. Since we don't care whether the closest value is over or under, we can just subtract that `ratio` from 1 and take the absolute value. Then all we have to do is `arrange()` by the resulting value:
```{r}
babynames %>%
  filter(name == "Jordan") %>%
  group_by(year) %>%
