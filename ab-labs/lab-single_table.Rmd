...
When in doubt, reference the data transformation [cheat sheet](https://www.rstudio.org/links/data_transformation_cheat_sheet). Pay careful attention to the pictures!

### Load in some data
We'll start by loading the `babynames` package, which contains data about names given to babies born in the United States beginning in 1880:
...
### `select()`
You can extract a subset of the **columns** using `select()`:
...
You'll notice that inside the `filter()` command is a **boolean statement** (hence the double equals sign). This is because `filter()` is akin to asking for all rows where a particular condition is true (in this case, where the value of `name` is equal to "Jordan").
...
Note that `jordan` is a `tbl_df`, a `tbl`, and a `data.frame`. Objects in R can have more than one type!

### Visualization (sneak peek)
Let's make a quick plot of the popularity of `Jordan` over time. To do this, we'll use the `ggplot2` package: 
...
#   - the data is contained in the 'jordan' object
#   - the 'year' column is mapped to the x-axis
#   - the 'n' column is mapped to the y-axis
ggplot(data = jordan, aes(x = year, y = n)) + #<- the '+' operator means we're adding to this plot
  # represent the data as a line chart,
  # break the data up by 'sex' and give each group its own color
  geom_line(aes(color = sex))
```
Pretty cool, huh? Don't worry too much about the details of constructing this plot, we'll have a whole workshop on data visualization in a couple of weeks :-)
...
Here, we summarize `jordan` to find the greatest number of `Jordan`s born in a single year (to a single sex):

```{r}
jordan %>%
  summarize(max_jordans = max(n))
```
...
```{r}
jordan %>%
  summarize(num_rows = n(), max_jordans = max(n))
```
